"""
Данный код выполняет распознавание и классификацию объектов (автомобилей) посредством использования библиотеки OpenCV для захвата видео с IP-камеры.
"""
...
if __name__ == '__main__':
    ...
    net = cv2.dnn.readNet("yolov3.weights", "yolov3.cfg")  # вместо yolov3-tiny.weights  подгрузить yolov3.weights !
    ...
    while True:
        try:
            ...
            while video_camera_capture.isOpened():
                ...
                frame = cv2.resize(frame, (int(1920 / 1.5), int(1080 / 1.5)))
                cv2.imshow("Parking Lot", frame)
                if cv2.waitKey(0):
                    break
